    ...
    if len(list_video) > 0:
        try:
            ...
            for video in list_video:
                ...
                video_file.get_contents_to_filename('/tmp/' + video_file.name)
                video_conv = video.originalVideoPath.name + '.conv.mp4'
                Popen(['ffmpeg', '-i', '/tmp/' + video_file.name,
                       '/tmp/media/' + video_conv]).wait()
                k = Key(bucket)
                k.key = 'media/' + video_conv
                k.set_contents_from_filename('/tmp/media/' + video_conv)
                video.state = 'CON'
